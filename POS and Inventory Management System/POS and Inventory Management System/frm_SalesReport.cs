...
namespace POS_and_Inventory_Management_System.CASHIER
{
    public partial class frm_SalesReport : Form
    {
        ...
        public void LoadSalesReport()
        {
            ...
            {
                cm = new SqlCommand("select TransactionDate,ProductCode, TransactionNo,  ProductName, category,price, totaltax,subTotal,qty,totalprice,discount,payMode from tblSales where TransactionDate between'" + dt1.Value.ToString("yyyy-MM-dd") + "' and'" + dt2.Value.ToString("yyyy-MM-dd") + "'", cn);
            }
            else
            {
                cm = new SqlCommand("select TransactionDate,ProductCode, TransactionNo, ProductName, category,price, totaltax,subTotal,qty,totalprice,discount,payMode from tblSales where TransactionDate between'" + dt1.Value.ToString("yyyy-MM-dd") + "' and'" + dt2.Value.ToString("yyyy-MM-dd") + "'and cashier like '" + cboCashier.Text + "'", cn);
            }
            dr = cm.ExecuteReader();
            ...
            while (dr.Read())
            {
                ...
                _total += double.Parse(dr["totalprice"].ToString());
