        ...
        try
        {
            ...
            foreach (var product in products)
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        int quantity = Convert.ToInt32(row.Cells[7].Value) + 1;
                        ...
                        row.Cells[7].Value = quantity;
                        decimal subtotal = product.SalePrice + product.SalePrice * product.Tax / 100;
                        row.Cells[6].Value = subtotal.ToString("#,##0.00");
                        decimal updatedTotalPrice = subtotal * quantity;
                        row.Cells[8].Value = updatedTotalPrice.ToString("#,##0.00");
                        itemFound = true;
                ...
                if (!itemFound)
                {
                    ...
                    dataGridView1.Rows.Add(product.ProductId, product.PCode, product.ProductName, product.Category, product.SalePrice, product.Tax, product.TotalPrice.ToString("#,##0.00"), 1, totalqtyprice.ToString("#,##0.00"));
                }
    ...
    public int StockInQty { get; set; }
        ...
        private string CalculateTotalQuantity()
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                {
                    int quantity = Convert.ToInt32(row.Cells[7].Value);
        ...
        private string CalculateTotalTax()
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (row.Cells[5].Value != null)
                {
                    ...
                    decimal tax = (Convert.ToDecimal(row.Cells[4].Value)*Convert.ToDecimal(row.Cells[5].Value)/100)*Convert.ToDecimal(row.Cells[7].Value);
        ...
        private string CalculateTotalPrice()
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (row.Cells[5].Value != null)
                {
                    decimal quantity = Convert.ToDecimal(row.Cells[4].Value) * Convert.ToDecimal(row.Cells[7].Value);
                    totalQuantity += quantity;
        ...
        private string CalculateSubtotal()
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                {
                    decimal quantity = Convert.ToDecimal(row.Cells[8].Value);
                    totalQuantity += quantity;
                }
                ...
                using (SqlConnection cn = new SqlConnection(dbcon.MyConnection()))
                {
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        if (dataGridView1.Rows[i].Cells[0].Value != null) // Ensure the row is not empty
                        {
                            ...
                            //using (SqlCommand cm = new SqlCommand("UPDATE tblProduct SET StockInQty = StockInQty - @qty WHERE PCode = @pcode", cn, transaction))
                            //{
                            //    cm.Parameters.AddWithValue("@qty", int.Parse(dataGridView1.Rows[i].Cells[7].Value.ToString()));
                            //    cm.Parameters.AddWithValue("@pcode", dataGridView1.Rows[i].Cells[1].Value.ToString());
                            //    cm.ExecuteNonQuery();
                            //}

                            // Insert into sales table
                            using (SqlCommand cm = new SqlCommand("INSERT INTO tblSales (TransactionNo, TransactionDate, bmonth, bmothyear, ProductCode, ProductName, category, price, tax, totalProductPrice, qty, totalPriceqty, subTotal, totaltax, totalprice, discount, grandTotal, payMode, receivedamount, balance, cashier) VALUES (@TransactionNo, @TransactionDate, @bmonth, @bmothyear, @ProductCode, @ProductName, @category, @price, @tax, @totalProductPrice, @qty, @totalPriceqty, @subTotal, @totaltax, @totalprice, @discount, @grandTotal, @payMode, @receivedamount, @balance, @cashier)", cn, transaction))
                            {
                                ...
                                cm.Parameters.AddWithValue("@totalProductPrice", dataGridView1.Rows[i].Cells[6].Value.ToString());
                                cm.Parameters.AddWithValue("@qty", dataGridView1.Rows[i].Cells[7].Value.ToString());
                                cm.Parameters.AddWithValue("@totalPriceqty", dataGridView1.Rows[i].Cells[8].Value.ToString());
                                cm.Parameters.AddWithValue("@subTotal", lbl_TotalPrice.Text);
                                cm.Parameters.AddWithValue("@totaltax", lbl_TotalTax.Text);
                                cm.Parameters.AddWithValue("@totalprice", lbl_subTotal.Text);
                                ...
                                cm.ExecuteNonQuery();
                                ReduceStock(dataGridView1.Rows[i].Cells[1].Value.ToString(), int.Parse(dataGridView1.Rows[i].Cells[7].Value.ToString()));
                            }
                        }
                    }
