    ...
    public partial class frm_ManageProduct : Form
    {
        ...
        public void LoadProduct()
        {
            try
            {
                ...
                cn.Open();
                cm = new SqlCommand("select PCode,ProductDate, ProductName,Category,CostPrice,Qty,Profit,SalesPrice, Tax,TotalPrice,StockInQty,Barcode from tblProduct where PCode like '%" + txt_Search.Text + "%'", cn);
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
                    i++;
        ...
        private void txt_Search_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    // Format the date as dd/MM/yyyy
                    string formattedFDate = fDate.ToString("dd/MM/yyyy");
                    dataGridView1.Rows.Add(i, dr["PCode"].ToString(), formattedFDate, dr["ProductName"].ToString(), dr["Category"].ToString(), dr["CostPrice"].ToString(), dr["Qty"].ToString(), dr["Profit"].ToString(), dr["SalesPrice"].ToString(), dr["Tax"].ToString(), dr["TotalPrice"].ToString(), dr["StockInQty"].ToString(), dr["Barcode"].ToString());
                }
                dr.Close();
                cn.Close();
            }
